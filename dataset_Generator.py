...
class Spline_Generator():
    ...
    def __init__(self, x_size=128, y_size=128):
        ...
        for i in range(config.max_point - config.min_point + 1):
            self.w_list.append(get_Spline_Weight_Matrix(
                i + config.min_point, config.n_degree, config.N))
...
def generate_variable_spline(verbose=0):
    ...
    f = h5py.File(config.dataset_path +
                  '/dataset_cc_variable_number_tmp' + '.hdf5', 'w')
    ...
    for idx in range(1, config.n_data + 1):
        ...
        if config.gen_verbose:
            plot_Spline_Img(img)
            plot_Spline_Curve_Control_Point(
                [point_pos], get_Spline_Weight_Matrix(n_point, config.n_degree, config.N))
        ...
        if idx % config.n_per_patch == 0:
            ...
            assert labels.shape == (
                config.n_per_patch, config.max_point, config.n_dim)
...
def generate_multiple_variable_spline(verbose=0):
    ...
    if not os.path.exists(config.dataset_path):
        os.makedirs(config.dataset_path)
    f = h5py.File(config.dataset_path +
                  '/dataset_cc_multiple_variable_spline_tmp' + '.hdf5', 'w')
    img_grp = f.create_group('imgs')
    label_grp = f.create_group('labels')
    ...
    for idx in range(1, config.n_data + 1):
        ...
        if config.gen_verbose:
            plot_Spline_Img(img)
            plot_Spline_Curve_Control_Point(
                [point_pos_matrix[i, :int(n_point_matrix[i])] for i in range(n_line)])
        ...
        if idx % config.n_per_patch == 0:
            ...
            assert imgs.shape == (config.n_per_patch, 128, 128)
            assert labels.shape == (
                config.n_per_patch, config.max_line, config.max_point, config.n_dim)
            # print(n_points.shape)
            print(labels.shape)
