...
trainset = Multi_Spline_Dataset_v1(path = config.dataset_path, n_all = config.n_data,
...
if config.continue_train:
	...
	net = Multi_Line_Model_v1(fix_pretrained = config.fix_pretrained, no_attention = False)
...
def train(epoch):
	...
	cul_clf_loss = 0
	n_batch = 0
	for batch_idx, (inputs, labels, line_masks, clf_line) in enumerate(trainloader):
