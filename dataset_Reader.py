...
class H5Reader:
	...
	def read_One(self, idx_patch, idx):
		...
		if self.multiple:
			# print(label.shape)
			assert label.shape == (config.max_line, config.max_point, config.n_dim)
...
class Multi_Vary_Spline_Dataset:
	...
	def __init__(self, path, n_all, verbose=0, train=True, train_part=0.7, transform=None):
		...
		self.reader = H5Reader(os.path.join(self.path, 'dataset_cc_multiple_variable_spline_500000'+'.hdf5'), multiple = True)
		np.random.seed(10)
	...
	def __getitem__(self, idx):
		if self.train: idx = self.img_list[idx]
		else: idx = self.img_list[idx+self.n_train]

		i_patch = int(idx/config.n_per_patch)
		idx_in_patch = idx%config.n_per_patch

		img, label, n_line, n_point = self.reader.read_One(i_patch, idx_in_patch)
		if config.reader_verbose:
			...
			for i_line in range(n_line):
				point_list.append(label[i_line, :int(n_point[i_line]), :].squeeze())
		...
		label = torch.from_numpy(label).view(config.max_line * config.max_point, config.n_dim)
		label = label / 128
		# get line mask array
...
if __name__ == '__main__':
	data = Multi_Vary_Spline_Dataset(config.dataset_path, config.n_data, verbose=1)
	for idx in range(10):
		# data[idx]
